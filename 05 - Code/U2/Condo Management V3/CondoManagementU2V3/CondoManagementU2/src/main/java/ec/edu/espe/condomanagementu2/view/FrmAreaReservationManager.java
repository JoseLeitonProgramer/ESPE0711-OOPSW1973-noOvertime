...
public class FrmAreaReservationManager extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jLabel2.setText("Ingresar ID del Residente: ");
    ...
    try {
        ...
        if (selectedRow != -1) {
            // Tomamos los datos de la fila seleccionada en la tabla
            String area = (String) tblReservations.getValueAt(selectedRow, 0);  // Área
            String dateStr = (String) tblReservations.getValueAt(selectedRow, 1);  // Fecha como String
            String status = (String) tblReservations.getValueAt(selectedRow, 2);  // Estado

            // Convertir la fecha String a Date
            SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");  // Asegúrate de que el formato coincida
            Date reservationDate = dateFormat.parse(dateStr);  // Parseamos la fecha desde String

            // Verificamos si los datos son válidos
            if (area == null || area.isEmpty() || reservationDate == null || status == null || status.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Por favor, selecciona una reserva válida.");
                return;
            }

            // Obtener el ID del residente desde el campo de texto en la GUI
            String residentId = txtResidentId.getText().trim();  // Obtener el ID desde el JTextField

            // Verificamos si el campo está vacío
            if (residentId.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Por favor, ingresa el ID del residente.");
                return;
            }

            // Obtener el documento del residente desde ResidentController usando el ID
            ResidentController residentController = new ResidentController();
            Document residentDoc = residentController.readResident(residentId);  // Obtener el documento del residente
            ...
            if (residentDoc != null) {
                ...
                updateTable();  // Actualiza la tabla para mostrar la nueva reserva
            } else {
                JOptionPane.showMessageDialog(this, "No se encontró al residente con el ID proporcionado.");
            }
        } else {
            JOptionPane.showMessageDialog(this, "Por favor, selecciona una fila para crear una reserva.");
        }
    } catch (Exception e) {
